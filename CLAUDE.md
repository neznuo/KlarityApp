...
| Audio capture | ScreenCaptureKit `SCStream` (system audio) + `AVCaptureSession` (microphone) |
| Audio mixing | `AVMutableComposition` + `AVAssetExportSession` — mixes two temp .m4a files into final `audio.m4a` after recording stops |
...
```
meetings/{meeting_id}/
  audio.m4a              # Mixed recording (system + mic, AAC stereo)
...
1. **Record** — `SCStream` captures system audio + `AVCaptureSession` captures microphone → each written to a separate temp `.m4a` via `SingleTrackWriter` (one `AVAssetWriterInput` per source, preventing AAC encoder format mismatch)
2. **Mix** — On stop, `DualTrackMixer` combines the two temp files into `audio.m4a` using `AVMutableComposition` + `AVAssetExportSession` (preset `AVAssetExportPresetAppleM4A`); falls back to system-audio-only if mic has no data
3. **Preprocess** — FFmpeg normalizes `audio.m4a` to 16kHz mono WAV
